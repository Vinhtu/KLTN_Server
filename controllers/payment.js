    ...
    try {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("data", (body) => {
          ...
          // exec(`open -a Safari ${JSON.parse(body).payUrl}`);



          // console.log(JSON.parse(body).payUrl, "JSON.parse(body).payUrl");
...
exports.result_payment = async (req, res) => {
  const { orderId, errorCode, localMessage } = req.body;
  ...
  if (errorCode === '0') {
    ...
    console.log("thanh toan thanh congo")
    res.send({ message: 'Payment received' });
  } else {
